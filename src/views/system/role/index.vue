    ...
    <el-form :model="queryParams" ref="queryRef" v-show="showSearch" :inline="true" label-width="68px">
      ...
      <el-form-item label="状态" prop="status">
        <el-select v-model="queryParams.status" placeholder="角色状态" clearable style="width: 240px">
          <el-option :key="'0'" :label="$t('dict.disable.1')" :value="'0'"></el-option>
          <el-option :key="'1'" :label="$t('dict.disable.0')" :value="'1'"></el-option>
        </el-select>
      </el-form-item>
  ...
  proxy.$refs["roleRef"].validate(valid => {
    if (valid) {
      if (form.value.roleId != undefined) {
        ...
        form.value.menuCheckStrictly = 1;
        form.value.deptCheckStrictly = 1;
  ...
  getRole(row.roleId).then(res => {
    ...
    nextTick(() => {
      deptTreeSelect.then(res => {
        nextTick(() => {
          if (deptRef.value) {
            deptRef.value.setCheckedKeys(res.checkedKeys === null || undefined ? [] : res.checkedKeys);
          }
        });
      });
    });
